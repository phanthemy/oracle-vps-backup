...
BACKUP_DIR="${1:-/var/backups/database}"
...
DBS=$(sudo -u postgres psql -t -A -c "SELECT datname FROM pg_database WHERE datistemplate = false AND datname != 'postgres';" 2>/dev/null || echo "")

for DB in $DBS; do
    echo "==> Backing up database: $DB..."
    sudo -u postgres pg_dump -d "$DB" -F c -b -v -f "${BACKUP_DIR}/${DB}_${TIMESTAMP}.dump" 2>/dev/null || true
    # Also keep plain SQL for easy inspection
    sudo -u postgres pg_dump -d "$DB" -F p > "${BACKUP_DIR}/${DB}_${TIMESTAMP}.sql" 2>/dev/null || true
done
